extends Node

signal meta_upgrade_currency_changed

const SAVE_FILE_PATH: String = "user://game.save"

var save_data: Dictionary = {
	"meta_upgrade_currency": 1000,
	"meta_upgrades": {}
}


func _ready() -> void:
	GameEvents.experience_vial_collected.connect(_on_experience_collected)
	load_save_file()


func load_save_file() -> void:
	if not FileAccess.file_exists(SAVE_FILE_PATH):
		return
	var file = FileAccess.open(SAVE_FILE_PATH, FileAccess.READ)
	save_data = file.get_var()
	print_debug(save_data)


func save() -> void:
	var file = FileAccess.open(SAVE_FILE_PATH, FileAccess.WRITE)
	file.store_var(save_data)


func emit_meta_upgrade_currency_changed() -> void:
	meta_upgrade_currency_changed.emit()


func add_meta_upgrade(upgrade: MetaUpgrade) -> void:
	if not save_data["meta_upgrades"].has(upgrade.id):
		save_data["meta_upgrades"][upgrade.id] = { "quantity": 0 }

	save_data["meta_upgrades"][upgrade.id]["quantity"] += 1
	save()


func get_meta_upgrade_amount(upgrade_id: String) -> int:
	if save_data["meta_upgrades"].has(upgrade_id):
		return save_data["meta_upgrades"][upgrade_id]["quantity"]
	return 0

func purchase_meta_upgrade(upgrade: MetaUpgrade) -> void:
	add_meta_upgrade(upgrade)
	_update_meta_currency(-upgrade.experience_cost)


func _update_meta_currency(amount: float) -> void:
	save_data["meta_upgrade_currency"] = \
		max(0, save_data["meta_upgrade_currency"] + amount)
	emit_meta_upgrade_currency_changed()
	save()


func _on_experience_collected(amount: float) -> void:
	_update_meta_currency(amount)
